...
// ── GET /api/settings/booking-credentials ────────────────────────────────────
// Returns which booking platforms are connected (email only — never the password).
router.get("/settings/booking-credentials", async (req, res) => {
  const userName = await authenticate(req, res);
  if (!userName) return;
  try {
    const { getBookingCredentials } = await import("../restaurants/bookingCredentialsManager.js");
    const creds = await getBookingCredentials(userName);
    res.json({
      openTable: {
        connected: !!(creds.openTableEmail && creds.openTablePassword),
        email:     creds.openTableEmail ?? null,
      },
      resy: {
        connected: !!(creds.resyEmail && creds.resyPassword),
        email:     creds.resyEmail ?? null,
      },
    });
  } catch (err) {
    logger.error({ err }, "[BookingCreds] GET failed");
    res.status(500).json({ error: "Failed to fetch booking credentials" });
  }
});

// ── POST /api/settings/opentable-credentials ─────────────────────────────────
router.post("/settings/opentable-credentials", express.json(), async (req, res) => {
  const userName = await authenticate(req, res);
  if (!userName) return;
  ...
  try {
    const { saveOpenTableCredentials } = await import("../restaurants/bookingCredentialsManager.js");
    await saveOpenTableCredentials(userName, email.trim(), password);
    res.json({ ok: true, email: email.trim() });
  } catch (err) {
    logger.error({ err }, "[BookingCreds] OpenTable save failed");
    res.status(500).json({ error: "Failed to save OpenTable credentials" });
  }
});

// ── DELETE /api/settings/opentable-credentials ───────────────────────────────
router.delete("/settings/opentable-credentials", async (req, res) => {
  const userName = await authenticate(req, res);
  if (!userName) return;
  try {
    const { clearOpenTableCredentials } = await import("../restaurants/bookingCredentialsManager.js");
    await clearOpenTableCredentials(userName);
    res.json({ ok: true });
  } catch (err) {
    logger.error({ err }, "[BookingCreds] OpenTable clear failed");
    res.status(500).json({ error: "Failed to clear OpenTable credentials" });
  }
});

// ── POST /api/settings/resy-credentials ──────────────────────────────────────
router.post("/settings/resy-credentials", express.json(), async (req, res) => {
  const userName = await authenticate(req, res);
  if (!userName) return;
  const { email, password } = req.body as { email?: string; password?: string };
  if (!email?.trim() || !password) {
  ...
  try {
    const { saveResyCredentials } = await import("../restaurants/bookingCredentialsManager.js");
    await saveResyCredentials(userName, email.trim(), password);
    res.json({ ok: true, email: email.trim() });
  } catch (err) {
    logger.error({ err }, "[BookingCreds] Resy save failed");
    res.status(500).json({ error: "Failed to save Resy credentials" });
  }
});

// ── DELETE /api/settings/resy-credentials ────────────────────────────────────
router.delete("/settings/resy-credentials", async (req, res) => {
  const userName = await authenticate(req, res);
  if (!userName) return;
  try {
    const { clearResyCredentials } = await import("../restaurants/bookingCredentialsManager.js");
    await clearResyCredentials(userName);
    res.json({ ok: true });
  } catch (err) {
    logger.error({ err }, "[BookingCreds] Resy clear failed");
    res.status(500).json({ error: "Failed to clear Resy credentials" });
  }
});
