...
// getCurrentDateTimeBlock has been moved to src/chat/getCurrentDateTimeBlock.ts
// This re-uses the imported version directly.
const getCurrentDateTimeBlock = _getCurrentDateTimeBlock;
...
const chatHandlerCore = async (req: Request, res: Response) => {
  ...
  if (isEmailRequest || isCalendarRequest || isCalendarWriteOp || isDeleteConfirm || isDeleteCancel || isEmailReplyFlowActive || isEmailReplyAccepted) {
    const emailCalResult = await handleEmailCalendar({
      sessionUserName,
      message,
      timezone,
      isEmailRequest,
    ...
    if (emailCalResult.hardcodedResponse !== undefined) {
      (req as any)._hardcodedResponse = emailCalResult.hardcodedResponse;
    }
    if (emailCalResult.emailPayload) {
      (req as any)._emailPayload = emailCalResult.emailPayload;
      broadcastToUser(sessionUserName, "email-compose", {
        type: "email_compose",
        ...emailCalResult.emailPayload,
      });
    }
    // Email/calendar block fully reassigns systemPrompt when it reads email/calendar.
    // For write-only ops it appends to systemPrompt instead.
    if (isEmailRequest || isCalendarRequest) {
      systemPrompt = emailCalResult.contextBlock;
    } else {
      systemPrompt += emailCalResult.contextBlock;
    }
  }
