...
export interface ClassifiedEmail {
  ...
  order?: {
    ...
    // "outbound" (default): retailer shipping something TO the user — the
    // original, only case this ever covered. "return": the user shipping
    // something BACK to a retailer (a return, trade-in, or mail-in kit) —
    // same tracking pipeline, just the opposite direction, with "refunded"
    // as its own further status once a refund-confirmation email arrives.
    direction: "outbound" | "return" | null;
    expectedDate: string | null;
    // Order total for an outbound email; the refund amount for a
    // direction="return" email reporting a refund was issued.
...
- "save_to_orders": shipping, delivery, or order status update from a retailer or carrier, for a PHYSICAL item that can be tracked, in EITHER direction. Set direction="outbound" for the retailer shipping something TO the user — an order confirmation, a "your item shipped," an "out for delivery," "your order is ready to ship," or a "delivered" notice. Set direction="return" for the user shipping something BACK to a retailer — a return label / "here's how to send your item back," a trade-in or mail-in kit ("send us your old device," "ship your trade-in within 30 days"), a "we've received your return" confirmation, or a refund-confirmation email ("your refund has been processed/issued"). A return follows the same lifecycle as an outbound order (tracking number, carrier, in transit, delivered — "delivered" for a return means the RETAILER received the package back, not the user) with one further status only returns ever reach: "refunded", set only when the email itself confirms money was actually returned — never infer or assume a refund from a "we received your return" email alone, that's still status="delivered". This is a broad category for physical goods — ANY email whose subject or body states a physical shipment/return/refund status update belongs here, even if the body is short, templated, or looks routine. A tracking number is NOT required — Amazon Logistics deliveries and many return labels especially often have no real carrier tracking number, but it's still worth tracking by order number and status language alone. Do not downgrade a physical order/return-status email to "fyi" or "none" just because it's short or looks automated — routine and low-effort is normal for these, not a sign it's noise. NEVER use "save_to_orders" for food — restaurant orders, pizza, delivery-app orders (Uber Eats, DoorDash, Grubhub, Postmates), coffee-shop mobile orders, or any prepared food being cooked and delivered/picked up directly by the vendor. Food is never shipped by a carrier and there's nothing meaningful to track — use "fyi" instead, regardless of whether the email has an order number or status language. This applies to any restaurant by name too, not just the well-known delivery apps — a local pizzeria's own order-confirmation email is still food, not a shippable order.
