...
- For road trips, every day where the traveler is driving to a new location MUST begin with a driving activity. The first day must include a drive FROM the traveler's home city to the first stop. The last day must include a drive FROM the final stop back to the traveler's home city. All middle days must include a drive from the previous stop to the next stop. Set the driving activity title to "Drive from [Origin] to [Destination]", include actual drive time and distance in the description, and For the first day set websiteUrl to https://www.google.com/maps/dir/?api=1&origin=Current+Location&destination=FIRST_HOTEL+CITY. For middle days set websiteUrl to https://www.google.com/maps/dir/?api=1&origin=PREVIOUS_HOTEL+CITY&destination=NEXT_HOTEL+CITY. For the last day set websiteUrl to https://www.google.com/maps/dir/?api=1&origin=LAST_HOTEL+CITY&destination=Current+Location. All hotel/city names are URL-encoded with + for spaces.
