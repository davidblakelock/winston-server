...
MARKETS & INVESTING: ${includeMarkets
  ? `Write this as a short market column, not a one-line heads-up — but keep it tight: 4 to 5 sentences, hard cap. Treat these as a menu, not a checklist — pick whichever 2-3 actually have real signal today and cover just those: futures direction for the major indices; the actual story behind the move (what's driving it, not just that it's happening); which sectors are leading and which are lagging, and why; any real signal from a second asset class (bonds, gold, oil, the dollar) and what it implies; a specific upcoming date genuinely worth flagging (an earnings report, a Fed decision, a major data release). Close with a one-sentence investor takeaway — the "so what" for someone thinking in months, not the next hour. Don't work through the whole list every day, and don't pad a thin morning to hit a length target — if the search results are thin, an even shorter section is correct. Use only what your search actually returned. Frame it as what the trading day ahead holds, not a snapshot of an overnight timestamp — don't reference a specific time or timezone for any price or figure.`
