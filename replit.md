...
- **Google Sign-In Auth** (primary, multi-device): Sign-in page shows "Continue with Google" button → full-page redirect to `/api/auth/google?signin=1` → Google OAuth → `/api/auth/callback?state=signin` → backend creates 30-day app session → redirects to `/?token=<token>&name=<name>&new=<0|1>` → frontend saves token to localStorage (`winston_session_token`) and clears URL params. `google_users` table maps Google sub IDs to `user_name`. `app_sessions` table stores sessions (30-day expiry). `setAuthTokenGetter` wired via `useAuth` hook so all API calls auto-include `Authorization: Bearer`. Chat POST requires valid session (401 if missing). Calendar/Gmail popup connect flow unchanged (uses `/api/auth/google` without `?signin=1`, `state=connect`). Legacy magic-link routes kept for backwards compat.
